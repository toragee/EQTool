...
/* 쓰기 전용. MISO 를 쓰지 않으므로 writeBytes 를 쓴다.
   덤으로 수신 버퍼(스택의 큰 가변 배열)가 필요 없어진다. */
...
void LCD_Init(void)
{
  ...
  LCD_WriteData(0x60);                    // MX | MV  -> landscape
#elif LCD_ROTATION == 180
  LCD_WriteData(0xC0);                    // MX | MY  -> portrait, upside down
#else
  LCD_WriteData(0xA0);                    // MY | MV  -> landscape, upside down
#endif
...
{ 
  uint16_t xs = Xstart + Offset_X, xe = Xend + Offset_X;
  uint16_t ys = Ystart + Offset_Y, ye = Yend + Offset_Y;

  // set the X (column) coordinates
